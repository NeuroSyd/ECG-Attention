...
class attentionLayer(Layer):
    ...
    def get_config(self):
        ...
        # conig.update({"units" : self.units})
        return config
        
##attention = attentionLayer()
##attention_config = attention.get_config()
##new_layer = Layer.from_config(attention_config)
...
# ----- Model ----- #
def get_model_attn(n_classes, last_layer='sigmoid'):
    kernel_size = 16
    kernel_initializer = 'he_normal'
    signal = Input(shape=(4096, 12), dtype=np.float32, name='signal')

    x = signal
    x = Conv1D(64, kernel_size, padding='same', use_bias=False,             kernel_initializer=kernel_initializer)(x)
    ...
    model_attn = Model(signal, diagn)
    
    # retrieve the config
    config = model_attn.get_config()
